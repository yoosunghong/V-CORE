...
평문 SOP를 cosine 검색하는 vector RAG만으로는 답할 수 없는, **엔티티 관계·실시간 스테이션 상태·과거 run KPI를 한 번에 조인해야 하는 복합(multi-hop) 운영 질의**를 typed graph로 처리합니다. "존 2에서 검사를 처리할 수 있는 스테이션과 각각의 마지막 병목률은?"처럼 토폴로지와 최신 지표를 동시에 따라가야 하는 질문에, 출처가 확인된 단일 응답을 반환하고 근거가 없으면 `not in the knowledge base`로 보류합니다.
...
-   **한국어 관계형 질의 파싱**: query grammar에 한국어 신호와 엔티티 별칭(`존 2`/`구역 2`->Zone B, `검사`->inspect, `적재`->load, `충전`->charge)을 추가해, 한국어 다중 홉 질의도 flat vector로 강등되지 않고 동일한 typed 경로를 타도록 했습니다.
...
-   **Cited / Abstain 계약**: 검색 결과를 신뢰되지 않은 컨텍스트로 sanitize한 뒤 문서 출처와 함께 주입하고, 검색된 절차를 모델 내부 기억보다 우선하며, 최소 score를 통과한 근거가 없으면 답변을 생성하지 않습니다.
...
#### 7\. Vector RAG로 풀리지 않는 복합 질의와 Hybrid Graph RAG 도입

-   **Problem**: Multilingual Vector RAG는 평문 SOP·사양 문서는 잘 검색했지만, "존 2에서 검사를 처리할 수 있는 스테이션과 각각의 마지막 병목률은?"처럼 **엔티티 관계(Zone-Station-Capability)와 최신 run KPI를 조인해야 하는 복합(multi-hop) 질의**는 평면 cosine 검색으로 답할 수 없었습니다. 그렇다고 모든 질의를 graph로 강제하면 자유 텍스트 SOP 검색 품질이 떨어집니다. 게다가 초기 graph 경로는 query grammar가 **영어 rule 기반**이라 한국어 다중 홉 질의가 조용히 flat vector로 강등됐고, KPI 근거를 **cell-global 병목률 한 줄**로만 보고해 서로 다른 존의 스테이션이 동일한 병목 수치를 받는 등 "그들 각각의 마지막 병목률" 질문의 절반이 누락됐습니다.
-   **Solution**: 질의 성격으로 검색 경로를 나누는 **Hybrid Graph RAG**를 채택했습니다. `is_relational_query`가 관계형으로 판정하면 `Cell -> Zone -> Station -> Capability`·`Cell -> Run -> Kpi` typed graph를 traversal하고, 그 외 자유 텍스트는 기존 vector path로 보냅니다. 분기는 `KnowledgeGateway` 포트 뒤 `HybridGraphKnowledgeGateway` 어댑터에 캡슐화해 LangGraph에는 단일 `retrieve` 노드만 노출했습니다. 한국어 질의를 위해 `is_relational_query`·`parse_zone`·`parse_capability`에 한국어 신호와 엔티티 별칭(`존 2`/`구역 2`→Zone B, `검사`→inspect, `적재`→load, `충전`→charge 등)을 추가했고, station별 KPI 귀속을 위해 `latest_zone_metric`을 도입해 run의 `zone_heatmap`에서 per-zone 병목률을 산출·부착하고 cell-global 값은 라벨링해 fallback으로 유지했습니다.
-   **Result**: 한국어·영어 복합 질의가 모두 graph 경로로 라우팅되어 해당 존·역량의 스테이션과 per-zone 병목률(`Last zone bottleneck_rate`)을 출처와 함께 반환하고, 근거가 없으면 보류합니다. 자유 텍스트 SOP 질의는 vector path를 그대로 유지해 두 검색 방식의 강점을 모두 살렸습니다. 한국어 질의·존별 귀속 회귀 테스트를 추가했으며, 이어서 graph가 매 요청마다 in-process 재구축되던 비용도 제거했습니다(아래 8번).

| 항목 | 개선 전 | 개선 후 |
| --- | --- | --- |
| 복합(multi-hop) 질의 | vector 평면 검색으로 응답 불가 | **typed graph traversal로 응답** |
| 검색 경로 | vector 단일 | **Hybrid (관계형→graph / 자유텍스트→vector)** |
...
-   **GraphRAG 표현력 제한**: 관계형 query grammar가 영어 rule 기반이었고 KPI 근거를 cell-global 병목률 한 줄로만 보고하던 문제를, 한국어 entity/constraint 파싱(`존 2`·`검사`·`병목률` 등)과 station별(zone별) KPI attribution(`zone_heatmap` 기반 per-zone 병목률)으로 개선했습니다. 또한 graph가 **매 요청마다 in-process 재구축되던 문제**를 `(stations, runs)` 콘텐츠 fingerprint 기반 memoization(상태 변경 시에만 재구축, LRU 캐시)으로 해소해 per-query 재구축 비용을 제거했습니다(해결 과정: [troubleshooting/graphrag-expressiveness.md](../troubleshooting/graphrag-expressiveness.md)). 프로세스 경계를 넘는 외부 영속 graph store(Neo4j/RDF)와 복합 제약 grammar는 CSP 규모의 향후 과제로 남습니다.
